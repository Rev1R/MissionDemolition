...
public class ProjectileLine : MonoBehaviour
{
    ...
    { 
        ...
        set
        {
            ...
            if ( _poi != null)
            {
                //Если поле _poi содержит действительную ссылку, сбросить все отальные параметры в исходное состояние
                line.enabled = false;
                points = new List<Vector3>();
                AddPoint();
            }
    ...
    public void AddPoint()
    {
        ...
        if (points.Count == 0)
        {
            //если это точка зауска..
            Vector3 launchPosDiff = pt - Slingshot.LAUNCH_POS;   //для определения добавить дополнительный фрагмент линии, чтобы помочь лучше прицелиться в будущем
    ...
    void FixedUpdate()
    {
        ...
        //если интересующий объект найден, добавить точку с его координатами в каждом FixedUpdate
        AddPoint();
        if (FollowCam.POI == null)
        {
